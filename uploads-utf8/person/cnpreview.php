...
'r_usergroup','r_openness','r_title','r_template','r_ability','m_logo','m_nameshow');
$sqlss='';
foreach($rsqlstr as $v) $sqlss.="$v,";
$sqlss=substr($sqlss,0,-1);
$rs=$db->get_one("select $sqlss from {$cfg['tb_pre']}resume INNER JOIN {$cfg['tb_pre']}member on r_member=m_login where r_member='$username' and r_id=$rid");
if($rs){
...
<tr>
  ...
  <td><?php echo $r_birth;?></td>
  <td width="130" rowspan="7" align="center" valign="middle"><img src="<?php echo $m_logo;?>" width="100" height="120" /></td>
</tr>
<tr>
...
<tr>
  ...
  <td><?php echo $r_height;?></td>
  <td align="right">体　　重：</td>
  <td><?php echo $r_weight;?></td>
...
<tr>
  <td width="80" align="right">工作岗位：</td>
...
<tr>
  ...
  <td colspan="3"><?php echo xreplace($r_trade);?></td>
</tr>
<tr>
  <td align="right">工作地区：</td>
  <td><?php echo xreplace($r_workadd);?></td>
...
<tr>
  <td colspan="4" align="center"><input type="button" name="Submit3" value="打印简历" class="inputs" onclick="window.print();" />
    
    <input type="button" name="Submit3" value="下载word文件" class="inputs" onclick="window.open('cnpreview.php?rid=<?php echo $r_id;?>&do=doc');" />
    
    <input type="button" name="Submit3" value="关闭窗口" class="inputs" onclick="window.close();" /></td>
</tr>
</table>