...
if($do=='pay'){
    ...
    if($rs){
        if($rs['c_usetime']=='0000-00-00 00:00:00'&&$rs['end']<0){
            ...
            showmsg("已充值[$rs[c_validnum]]成功，您现在可以使用充值卡消费了！","?mpage=person_onlinepay&show=$show",0,2000);exit();
        }else{
...
<div class="memrightl">
    ...
    <div class="memts">
        <li><font color="#FF0000"><b>温馨提示：</b></font>您账户当前余额为 <font color="#FF0000"><b><?php echo $Mbalance;?></b></font> ，您可以通过”<a href="?mpage=person_onlinepay&show=$show&p=onlinepay">在线支付</a>“或”<a href="?mpage=person_onlinepay&show=$show&p=cardpay">充值卡充值</a>“对账户进行充值。</li>
    </div>
    <?php if($p=='onlinepay'){?>
...
<form name="payform" action="?do=pay&mpage=person_onlinepay&show=<?php echo $show;?>" method="post" onSubmit="return check();">
  <tr class="memtabhead" align="center">
