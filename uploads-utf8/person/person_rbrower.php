...
if($do=='del'){
	...
	showmsg('删除成功！',"?mpage=person_rbrower&show=$show",0,2000);exit();
}else{
    $db->get_one("delete from {$cfg['tb_pre']}rbrower where r_type!=1 and DATEDIFF('".date('Y-m-d')."',r_adddate)>30");
    $rsdb=array();$sqladd='';
    $rid=intval($rid);$rid&&$sqladd=" and r_bid=$rid";
    $sql="select r_id,r_bid,r_adddate,r_name from {$cfg['tb_pre']}rbrower where r_bmember='$username' and r_type=1 $sqladd order by r_adddate desc";
    $query=$db->query($sql);
    $counts = $db->num_rows($query);
    $page= isset($_GET['page'])?$_GET['page']:1;//默认页码
    $getpageinfo = page($page,$counts,"index.php?mpage=person_rbrower&show=$show",20,5);
...
?>
<div class="memrightl">
    <div class="memrighttit"><span></span>简历被浏览记录</div>
...
全选&nbsp; &nbsp; <input name="button3" type="button" class="inputs" value="删除" onClick="confirmX(1);"></div><?php echo $getpageinfo['pagecode'];?></td>
      ...
      <script language="javascript">
        function confirmX(num)
        {
        	...
        	if(num==1)
        	{
        		document.form.action="?do=del&mpage=person_rbrower&show=<?php echo $show;?>";
