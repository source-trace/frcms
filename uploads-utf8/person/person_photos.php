...
if($do=='savedata'){
    for($i=1;$i<=5;$i++){
        ...
        if ($filename!=''){
            ...
            $db ->query("INSERT INTO {$cfg['tb_pre']}picture (p_filename,p_type,p_status,p_member,p_adddate,p_name,p_flag,p_info) VALUES('$filename',22,1,'$username',NOW(),'$name','$flag','$info')");
        }
    }
	showmsg('照片保存成功！',"?mpage=person_photos&show=$show",0,2000);exit();
}elseif($do=='hidden'){
	$db ->query("update {$cfg['tb_pre']}picture set p_status=0 where p_member='$username' and p_id in ($checks)");
	showmsg('设置屏蔽成功！',"?mpage=person_photos&show=$show",0,2000);exit();
}elseif($do=='show'){
	$db ->query("update {$cfg['tb_pre']}picture set p_status=1 where p_member='$username' and p_id in ($checks)");
	showmsg('设置显示成功！',"?mpage=person_photos&show=$show",0,2000);exit();
}elseif($do=='del'){
	...
	showmsg('删除成功！',"?mpage=person_photos&show=$show",0,2000);exit();
}else{
    $sql="select p_id,p_filename,p_status,p_member,p_adddate,p_name,p_flag,p_info from {$cfg['tb_pre']}picture where p_member='$username' and p_type=22 order by p_adddate desc";
    $query=$db->query($sql);
    $counts = $db->num_rows($query);
    $page= isset($_GET['page'])?$_GET['page']:1;//默认页码
    $getpageinfo = page($page,$counts,"index.php?mpage=person_photos&show=$show",20,5);
...
<div class="memrightl">
    <div class="memrighttit"><span></span>职场风采管理</div>
    <div class="memrightbox">
    <table width="100%" border="0" align="center" cellpadding="4" cellspacing="0" class="memtab" id="perpic">
        <form name="picform" action="?mpage=person_photos&show=<?php echo $show;?>" method="post">
        <tr class="memtabhead">
...
function confirmX(num)
{
	...
	if(num==1)
	{
		document.picform.action="?do=show&mpage=person_photos&show=<?php echo $show;?>";
		document.picform.submit();
	}
	if(num==2)
	{
		document.picform.action="?do=hidden&mpage=person_photos&show=<?php echo $show;?>";
		document.picform.submit();
	}
	if(num==3)
	{
		document.picform.action="?do=del&mpage=person_photos&show=<?php echo $show;?>";
		document.picform.submit();
	}
	return false;	
}
function checkAll(box1) {
    ...
    <div id="loading" style="z-index: 9999; display:none; position:absolute; text-align: center;width:100%;height:100px;">上传中，请稍后……<br /><br /><img src="../images/loading.gif" /></div>
      <div class="memts">
        <li><font color="#FF0000"><b>温馨提示：</b></font><span class="tdcolor">照片文件类型支持jpg、gif、png。</span>点“浏览”上传照片，上传后填写照片名称和简介。可同时保存5张照片，不足5张多余项留空即可。</li>
    </div>
      <table width="100%" border="0" align="center" cellpadding="4" cellspacing="0">
        <tr>
          <td colspan="2" align="center"><table width="100%" style="border:#D3EEFC 1px solid" align="center" cellpadding="2" cellspacing="0" class="mtable">
            <form action="?do=savedata&mpage=person_photos&show=<?php echo $show;?>" method="post"  name="addform" id="addform">
              <?php for($i=1;$i<=5;$i++){?>
              <tr>
