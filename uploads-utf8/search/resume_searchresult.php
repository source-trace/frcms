...
$strfilename="resume_searchresult.php?position=$position&workadd=$workadd&numberss=$numbers&taxiss=$taxis&usergroup=$usergroup&item1=$item1&item2=$item2&item3=$item3&item4=$item4&item5=$item5&keyword=$keyword";
$pagesize=$numbers;
$sqladd='';
if($trade!=''){
    $sqladd.=" and (";
...
if(hireprofession($profession)!='') $sqladd.=" and r_zhicheng like '%".hireprofession($profession)."%'";
...
if($item5){
    $sqladd.=" and DATEDIFF('".date('Y-m-d')."',r_adddate)<=$item5";
}else{
    if($datetime) $sqladd.=" and DATEDIFF('".date('Y-m-d')."',r_adddate)<=$datetime";
}
if($keyword!='') $sqladd.=" and (r_position like '%$keyword%' or r_appraise like '%$keyword%' or r_name like '%$keyword%' or r_request like '%$keyword%' or r_title like '%$keyword%')";
...
$sqladd=" r_flag=1 and r_cnstatus=1 and r_openness=0 and m_flag=1 and DATEDIFF(m_startdate,'".date('Y-m-d')."')<=0 and DATEDIFF(m_enddate,'".date('Y-m-d')."')>=0 and r_personinfo=1 and r_careerwill=1".$sqladd;
...
$newresumelist.="        <tr class=\"moretabmain\" align=\"center\" onmouseover=\"this.style.background='#F7F8FE'\" onmouseout=\"this.style.background=''\">\r\n";
...
include template('resume_searchresult','search');
?>