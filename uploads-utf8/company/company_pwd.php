...
if($do=='savelogin'){
	if(empty($login)||empty($pass)){
		...
		if($login==$username){
			...
			if($rs){
				$db ->query("update {$cfg['tb_pre']}myexpert set m_cmember='$login' where m_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}interview set i_cmember='$login' where i_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}myreceive set m_cmember='$login' where m_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}recycle set r_cmember='$login' where r_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}myinterview set i_cmember='$login' where i_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}dept set d_cmember='$login' where d_cmember='$username'");
				$db ->query("update {$cfg['tb_pre']}hire set h_member='$login' where h_member='$username'");
				...
				showmsg('用户名修改成功!',"?mpage=company_pwd&show=$show",0,2000);exit();
			}else{
		...
		if($opwd==$pwd){
			...
			if($pwd!=$repwd){
				...
				if($rs){
                    ...
                    showmsg('用户密码修改成功！',"?mpage=company_pwd&show=$show",0,2000);exit();
                exit();
				}else{
					showmsg('原密码错误，请重新输入！',"-1",0,2000);exit();
				}
...
<div class="memrightl">
    ...
    <div class="memrightbox">
        ...
        <form method="post" name="loginform" id="loginform" action="?do=savelogin&mpage=company_pwd&show=<?php echo $show;?>">
        <ul>
        ...
        <?php }?>
        <form method="post" name="pwdform" id="pwdform" action="?do=savepwd&mpage=company_pwd&show=<?php echo $show;?>">
        <ul>
        	<li class="t">修改密码</li>
        	<hr />
            <li class="l"><span style="color:#F00">*</span> 原密码：</li>
