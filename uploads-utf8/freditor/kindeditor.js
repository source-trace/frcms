...
KE.setting = {
	...
	items : [
		...
		'flash', 'media', 'accessory', 'advtable', 'hr', 'emoticons', 'link', 'unlink', '|', 'pagebreak', '|', 'about'
	],
	colorTable : [
...
KE.util = {
	...
	getData : function(id, wyswygMode) {
		...
		//替换分页修改
		return this.toData(id, g.iframeDoc.body.innerHTML).replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!-- pagebreak -->");
	...
	getSrcData : function(id) {
		...
		//替换分页修改
		return g.iframeDoc.body.innerHTML.replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!-- pagebreak -->");
...
//            return false;
			...
			var onloadFunc = function() {
				...
				if (typeof data === 'object' && 'error' in data) {
					if (data.error === 0) {
						var ext = data.url.substring(data.url.lastIndexOf('.')+1, data.url.length).toLowerCase();
						self.insert(id, data.url, title, ext);
					} else {
						alert(data.message);
						return false;
					}
  ...
  {
  case "doc":
   img=g.pluginsPath + 'accessory/doc.gif';
  break;
  case "xls":
   img=g.pluginsPath + 'accessory/xls.gif';
  break;
  case "ppt":
   img=g.pluginsPath + 'accessory/ppt.gif';
  break;
  case "rar":
   img=g.pluginsPath + 'accessory/rar.gif';
  break;
  case "zip":
   img=g.pluginsPath + 'accessory/zip.gif';
  break;
  case "txt":
   img=g.pluginsPath + 'accessory/txt.gif';
  break;
  case "pdf":
   img=g.pluginsPath + 'accessory/pdf.gif';
  break;
  default:
   img=g.pluginsPath + 'accessory/attach.gif';
  }
  var html = '<a href="' + url + '" >';
...
KE.plugin['source'] = {
	click : function(id) {
		...
		if (!g.wyswygMode) {
			//分页修改
			KE.util.setFullHtml(id, g.newTextarea.value.replace(/<!-- pagebreak -->/ig,'<div class="pagebreak">pagebreak</div>'));
			...
			KE.hideMenu(id);
			//分页修改
			g.newTextarea.value = KE.util.getData(id).replace(/<div?[^>]*?>pagebreak<\/div\s?>/ig, "<!-- pagebreak -->");
